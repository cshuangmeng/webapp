			...
			$.ajax({
			    ...
			    success: function(data){
			    	if(data.code == 0){
			    		...
			    		window.location.href = "/weixin/bangdingPhone.html?openid="+config.openid+"&goodId="+config.goodId+"&loginReturn=3&isInviteCode="+config.isInviteCode+"&unionId="+config.unionId;
			    	}else{
			    		$tipContent.text(data.msg);
						$('#js-tipCon').css('display','block');
						$('#js-tip').css("margin-top",(window.screen.height/3)+"px");

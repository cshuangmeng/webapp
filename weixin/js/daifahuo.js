$(function(){
	var $daifahuomain = $('#js-p-daifahuo-main');
	var config = {};
	//获取url携带的参数
	...
	$.ajax({
	    type: "POST",
	    url: orderlist,
	    data:{"uuid":config.uuid,"state":"1"},
	    dataType:"json",
	    async:false,
	    success: function(data){
	    	if(data.code == 0){
	    		for(var i=0;i<data.data.orders.length;i++){
	    			$daifahuomain.append('<div class="p-daifahuo-li"><div class="orderNumber"><span>订单编号</span><span>'+data.data.orders[i].tradeNo+'</span></div><div class="js-goodsList'+i+'"></div><div class="mes"><span>商品共计实付  ￥<b>'+data.data.orders[i].totalPrice+'</b></span><div class="clear"></div></div></div>');
